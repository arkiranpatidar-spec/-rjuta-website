...
- `intelligence.html` — Haryana real-estate market intelligence.
- `brokers.html` — Gurugram BrokerCheck and source-linked regulatory passports.
- `scripts/update-brokers.mjs` — reproducible Haryana RERA snapshot normalizer.

## Refresh BrokerCheck data

Run `node scripts/update-brokers.mjs` to rebuild from the saved research snapshots. Run `node scripts/update-brokers.mjs --live` to retrieve the current official pages. Live retrieval and public reproduction must only be used after confirming permission under Haryana RERA's terms. The included GitHub workflow is manual-only for that reason.
